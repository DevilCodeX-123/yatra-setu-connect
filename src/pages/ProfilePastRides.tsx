...
function EcoBanner({ totalKm, rides }: { totalKm: number; rides: number }) {
    ...
    return (
        <div className="rounded-2xl overflow-hidden shadow-lg shadow-emerald-100 mb-6">
                ...
                <div className="relative flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-emerald-200 text-[10px] font-black uppercase tracking-widest">Green Traveler</span>
                        </div>
                        <h2 className="text-2xl font-black italic uppercase tracking-tighter text-white">Your Eco Impact</h2>
            ...
            {/* Stats grid */}
            <div className="grid grid-cols-2 sm:grid-cols-4 bg-white border-x border-b rounded-b-2xl border-emerald-100">
                {[
                    { Icon: Wind, color: "text-green-600", bg: "bg-green-50", val: `${co2} kg`, label: "CO₂ Saved" },
                    { Icon: Leaf, color: "text-teal-600", bg: "bg-teal-50", val: trees, label: "Trees Equiv." },
                    { Icon: Droplets, color: "text-blue-600", bg: "bg-blue-50", val: `${nox}g`, label: "NOx Avoided" },
                    { Icon: Zap, color: "text-orange-500", bg: "bg-orange-50", val: cars, label: "Car Trips Saved" },
                ].map((s, i) => (
                    <div key={i} className="p-4 text-center border-r last:border-r-0 border-emerald-50 flex flex-col items-center gap-1.5">
            ...
            {/* Footer note */}
            <div className="bg-emerald-50 border border-emerald-100 rounded-b-2xl -mt-1 px-5 py-2.5 flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-3" onClick={onClose}>
            <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-slate-900 px-6 py-5 flex items-center justify-between">
                    <div>
                        <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mb-0.5">Ride Feedback</p>
                        <h3 className="text-white font-black text-base italic uppercase">
                            {ride.from} → {ride.to}
                        </h3>
                    </div>
                ...
                <div className="flex border-b bg-slate-50">
                    {([
                        ...
                        <button key={t.id} onClick={() => setTab(t.id)}
                            className={`flex-1 py-3 flex flex-col items-center gap-1 text-[9px] font-black uppercase tracking-widest transition-colors border-b-2
                                ${tab === t.id ? "border-primary text-primary bg-white" : "border-transparent text-slate-400 hover:text-slate-600"}`}>
                ...
                <div className="p-6 space-y-4">
                    ...
                    {tab === "rate" && (
                        <div className="space-y-4">
                            <div className="text-center">
                                <p className="text-sm font-bold text-slate-500 mb-4">How was your journey?</p>
                                <div className="flex justify-center mb-2">
                                    <StarRating value={rating} onChange={setRating} />
                                </div>
                                <p className="text-xs font-bold text-amber-500 h-4">{labels[rating]}</p>
                            </div>
                            <textarea
                                className="w-full border border-slate-100 bg-slate-50 rounded-2xl p-4 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 h-20"
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {([
                                    ...
                                    <button key={o.id} onClick={() => setAgainst(o.id)}
                                        className={`p-3 rounded-2xl border-2 text-left transition-all ${against === o.id ? "border-primary bg-primary/5" : "border-slate-100 hover:border-slate-200"}`}>
                                        <div className="text-xl mb-1">{o.emoji}</div>
                                        <p className="text-[10px] font-black uppercase tracking-wider text-slate-600">{o.label}</p>
                                    </button>
                                ))}
                            </div>
                            <textarea
                                className="w-full border border-slate-100 bg-slate-50 rounded-2xl p-4 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 h-24"
                    ...
                    <Button onClick={submit} disabled={loading}
                        className="w-full h-12 rounded-2xl font-black uppercase italic tracking-wide gap-2 shadow-lg shadow-primary/20">
                        {loading
    ...
    return (
        <DashboardLayout title="Past Rides" subtitle="Completed journeys & your eco impact" sidebarItems={sidebar}>
            <div className="max-w-4xl pb-20 md:pb-0">
                {/* Demo notice */}
                <div className="mb-5 flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-2xl">
                ...
                <div className="flex items-center justify-between mb-4">
                    <h2 className="flex items-center gap-2 text-base font-black italic uppercase tracking-tighter text-slate-800">
                        <History className="w-4 h-4 text-slate-400" /> Completed Rides
                    </h2>
                ...
                <div className="space-y-4">
                    {SAMPLE_RIDES.map(ride => {
                        ...
                        return (
                            <div key={ride.id} className="bg-white border border-slate-100 rounded-3xl shadow-sm shadow-slate-100 hover:shadow-md hover:shadow-slate-200 transition-all overflow-hidden">
                                {/* Card header strip */}
                                <div className="px-5 py-3 bg-slate-50 border-b flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-500">₹{ride.amount}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {userRating > 0 && (
                                            <div className="flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">
                                                <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                                                <span className="text-[9px] font-black text-amber-500">{userRating}.0</span>
                                            </div>
                                        )}
                                        <span className="text-[9px] font-bold text-slate-300">PNR {ride.pnr}</span>
                                    </div>
                                </div>
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        <div>
                                            <p className="text-xl font-black text-primary italic leading-none">{ride.from}</p>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1">{ride.departure}</p>
                                        </div>
                                        <div className="flex-1 flex flex-col items-center gap-1">
                                            ...
                                            <p className="text-[9px] font-bold text-slate-400">{ride.km} km</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-xl font-black text-primary italic leading-none">{ride.to}</p>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1">{ride.arrival}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                                            <Calendar className="w-3.5 h-3.5 text-slate-300" /> {ride.date}
                                        </span>
                                        <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                                            <User2 className="w-3.5 h-3.5 text-slate-300" /> {ride.passengers} pax
                                        </span>
                                        <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                                            <MapPin className="w-3.5 h-3.5 text-slate-300" /> Seat {ride.seats}
                                        </span>
                                        <span className="flex items-center gap-1.5 text-[10px] font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        <Button size="sm" onClick={() => setModal(ride)}
                                            className="h-9 px-5 rounded-2xl text-[10px] font-black uppercase italic tracking-wide gap-1.5 bg-amber-500 hover:bg-amber-600 border-0 shadow-md shadow-amber-200">
                                            <Star className="w-3.5 h-3.5" />
                                            {userRating ? "Edit Feedback" : "Rate & Feedback"}
                                        </Button>
                                        <Button size="sm" variant="outline"
                                            onClick={() => setExpanded(open ? null : ride.id)}
                                            className="h-9 px-4 rounded-2xl text-[10px] font-black uppercase italic tracking-wide gap-1.5 border-slate-200">
                                    ...
                                    {open && (
                                        <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 gap-2.5">
                                            {[
                                                { label: "Operator", value: ride.operator, bg: "bg-slate-50", txt: "text-slate-700" },
                                                { label: "Bus No.", value: ride.busNo, bg: "bg-slate-50", txt: "text-slate-700" },
                                                { label: "CO₂ Saved", value: `${co2} kg`, bg: "bg-green-50", txt: "text-green-700" },
                                                { label: "NOx Avoided", value: `${nox}g`, bg: "bg-teal-50", txt: "text-teal-700" },
                                            ].map((item, i) => (
