    ...
    <Layout>
      <div className="max-w-2xl mx-auto px-4 py-12 pb-24 min-h-screen bg-background">
        <div className="mb-10 animate-fade-in">
          <h1 className="text-3xl sm:text-4xl text-premium text-primary leading-tight font-black ">Verify Ticket</h1>
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-[10px] font-black tracking-[0.3em] text-muted-foreground opacity-60 flex items-center gap-2">
              ...
              Official National Verification System
            </p>
          </div>
        </div>
            ...
            {ticketData ? (() => {
              ...
              const getStatus = () => {
                try {
                  ...
                  return { ...common, sub: `${diff} DAYS UNTIL TRAVEL` };
                } catch (e) {
                  return {
                    label: "CONFIRMED",
                    color: "bg-emerald-500 text-white shadow-emerald-500/20",
                ...
                <div className="bg-card border border-border overflow-hidden shadow-elevated rounded-[40px] print:shadow-none print:border print:m-0 print:rounded-none printable-ticket">
                  ...
                  <div className={`px-8 py-8 flex items-center gap-6 border-b border-border relative ${status.headerBg}`}>
                    ...
                    <div className="flex-1">
                      <p className={`text-[10px] font-black tracking-[0.3em] mb-1 opacity-60 ${status.textColor}`}>Certificate Verified</p>
                      <h3 className={`text-2xl font-black leading-none ${status.textColor}`}>{status.label} BOOKING</h3>
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-4 mb-10 p-8 rounded-[32px] bg-secondary/50 border border-border relative overflow-hidden group">
                      ...
                      <div className="flex-1 text-center md:text-left z-10 w-full">
                        <p className="text-[10px] font-black text-muted-foreground tracking-[0.2em] mb-2 opacity-50">Origin / Departure</p>
                      ...
                      <div className="w-full md:w-32 flex flex-col items-center justify-center px-4 py-2">
                        ...
                        <p className="text-[8px] font-black text-muted-foreground mt-3 tracking-[0.3em] opacity-40">EXPRESS LINE</p>
                      </div>

                      <div className="flex-1 text-center md:text-right z-10 w-full">
                        <p className="text-[10px] font-black text-muted-foreground tracking-[0.2em] mb-2 opacity-50">Destination / Arrival</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-4">
                      {[
                        { icon: User, label: "Passenger(s)", value: ticketData.passengers[0]?.name + (ticketData.passengers.length > 1 ? ` +${ticketData.passengers.length - 1}` : ""), color: "text-primary" },
                        { icon: QrCode, label: "PNR Reference", value: ticketData.pnr, color: "text-primary-light" },
                        { icon: Bus, label: "Fleet ID", value: ticketData.bus?.busNumber, color: "text-foreground" },
                        { icon: MapPin, label: "Seat Assignment", value: ticketData.passengers.map((p: any) => p.seatNumber).join(", "), color: "text-emerald-500" },
                        { icon: Clock, label: "Validated On", value: format(new Date(ticketData.createdAt), 'dd MMM, hh:mm a'), color: "text-foreground" },
                        { icon: CheckCircle, label: "Fare Paid", value: `₹ ${ticketData.amount}`, color: "text-primary" },
                      ].map(item => (
                  ...
                  <div className="px-8 py-8 bg-secondary border-t border-border flex flex-col sm:flex-row gap-4 print:hidden">
                    ...
                    >
                      ...
                      {copied ? "GATEWAY LINK COPIED" : "SHARE CONFIRMATION"}
                    </Button>
                  </div>
                </div>
              );
              ...
              <div className="bg-red-500/5 border border-red-500/20 p-16 flex flex-col items-center text-center rounded-[40px] shadow-2xl animate-fade-in relative overflow-hidden">
                ...
                <h3 className="font-black text-red-500 text-3xl mb-3 ">Verification Failed</h3>
                <p className="text-[10px] font-black text-muted-foreground opacity-60 max-w-xs mb-10 ">
                  The reference <span className="text-foreground border-b border-muted-foreground">"{pnr}"</span> could not be authenticated against our active database. Please re-check the PNR on your official sms/email.
                </p>
